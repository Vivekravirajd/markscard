...
bnmit = Flask("bnmit", static_url_path="/static", static_folder="static")
...
@bnmit.route("/send-zip", methods=["POST"])
def send_zip():
...
@bnmit.route("/send-mail", methods=["GET"])
def send_mail():
...
@bnmit.route("/generate-report", methods=["POST"])
def make_data_frame():
...
@bnmit.route("/upload-file", methods=["POST"])
...
@bnmit.route("/change-mail", methods=["POST"])
...
from bnmit import bnmit
# Start the flask app
if __name__ == "bnmit":
    bnmit.run(host="0.0.0.0", port=8000)